...
| 7 | schema と validation scripts | `schemas/`, `scripts/validation/` | まだ不可 | schema inventory と validation command docs | frontmatter、safety term、manifest、source traceability checks が存在 | operator が clinical values 非埋め込みを確認 | clinical numeric values を schema に配置 |
...
| 11 | derived README / instructions / manifest / tests | `derived/custom_gpt_knowledge_package/` operator surfaces | reviewed knowledge files に限り可 | README、instructions、tests、audit、manifest | upload target が明示維持 | operator が upload / non-upload split を確認 | Knowledge target を silent expansion |
| 12 | Preview protocol | `derived/custom_gpt_knowledge_package/tests/` | knowledge export ではない | Preview protocol と dangerous-question tests | fail/pass/must-have/must-not-have が存在 | Preview run が manual で記録済み | Preview tests omitted |
| 13 | pharmacist red-flag review | `derived/custom_gpt_knowledge_package/audit/` | red-flag review 後のみ可 | checklist と quarantine file | unapproved content が quarantine 済み | pharmacist sign-off または unresolved status | unreviewed content exported |
| 14 | 最終監査 | integrated と derived package surfaces | reviewed scope に限り可 | final audit report と updated manifests | existence、frontmatter、manifest、traceability、dangerous-term、numeric assertion checks が通る | pharmacist と operator の review status 完了 | unresolved gate が一つでも残る |
