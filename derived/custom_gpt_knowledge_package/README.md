...
Custom GPT（GPT-5.5 運用想定）の **Knowledge 正本は本 package の `knowledge/` 13 ファイルのみ** である。ワークスペース sibling の `references/neurosurgery_safe_rag_pmda_product_source_register_resolved/`（306 files）は、PMDA 製品単位確認・safe-boundary 薬剤プロファイル・source register を進める **reference 作業用 corpus** であり、306 件を Knowledge へ一括投入する設計ではない。

reference 側の現状事実（本 package に写し込まない）: 127 薬剤は candidate inventory（施設未確定）、PMDA 製品 URL 確認済み 0/127、`custom_gpt_upload_safe: false`。reference 内で Custom GPT 直接候補とされるのは `09_MANIFESTS/custom_gpt_upload_manifest.csv` の 6 行のみで、いずれも human review pending である。

本 package への統合は `manifest/reference_migration_decision_ledger.csv` の migration_mode に従う。adapted_port は境界・運用言語の適応であり、医療内容の無批判コピーではない。212 薬剤プロファイルは integrated drug-profile 層が整うまで hold する。

**Ledger 306/306:** slice B（25 件 port 適応）と slice C（281 件 non-port 監査完走）済み。306 件すべてに decision があることは、REFERENCE を TARGET へ無批判コピーしたことを意味しない。
...
17. [manifest/future_policy_integration_matrix.md](manifest/future_policy_integration_matrix.md)
18. [manifest/custom_gpt_upload_manifest.csv](manifest/custom_gpt_upload_manifest.csv)
19. [tests/validate_upload_manifest.py](tests/validate_upload_manifest.py)
20. [manifest/knowledge_chunk_review_crosswalk.csv](manifest/knowledge_chunk_review_crosswalk.csv)
21. [tests/validate_unsafe_patterns.py](tests/validate_unsafe_patterns.py)
22. [manifest/knowledge_quarantine_register.csv](manifest/knowledge_quarantine_register.csv)
23. [tests/validate_quarantine_integrity.py](tests/validate_quarantine_integrity.py)
24. [tests/report_preview_promotion_candidates.py](tests/report_preview_promotion_candidates.py)
25. [tests/apply_preview_promotion.py](tests/apply_preview_promotion.py)
26. [tests/validate_review_state_integrity.py](tests/validate_review_state_integrity.py)
27. [tests/validate_release_readiness.py](tests/validate_release_readiness.py)
28. [manifest/reference_migration_decision_ledger.csv](manifest/reference_migration_decision_ledger.csv)
29. [tests/validate_reference_migration_ledger.py](tests/validate_reference_migration_ledger.py)
30. [manifest/facility_confirmation_status_ledger.csv](manifest/facility_confirmation_status_ledger.csv)
31. [tests/validate_facility_confirmation_status.py](tests/validate_facility_confirmation_status.py)
32. [manifest/derived_export_candidate_ledger.csv](manifest/derived_export_candidate_ledger.csv)
33. [manifest/integrated_origin_reclassification_summary.csv](manifest/integrated_origin_reclassification_summary.csv)
34. [tests/validate_derived_export_candidate_ledger.py](tests/validate_derived_export_candidate_ledger.py)
