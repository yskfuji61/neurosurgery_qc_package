...
2026-06-05 時点で、人間レビュー基準に沿った日本語補足 patch を **一部適用済み**（`knowledge/01`, `03`, `04`, `05`, `08`, `09`, `13`）。round 1（5 件）・round 2（4 件）・round 3（5 件）を適用済み。round 3 の要点:

- `upload 可否`、`Reference corpora`、`reference-only / hold` を日本語医療文書として誤読されにくい形へ（`01`）
- rFVIIa 節の `routine` を一律候補化しない表現へ（`03`）
- VTE 節の `中核` を重要な確認ノートへ（`04`）
- TICH-2 節の `routine` を海外資料から国内一律運用への短絡防止へ（`09`）
- `release readiness` を公開可能状態（release readiness）と境界文で補足（`13`）

修正は日本語補足と安全境界の明確化に限定しています。用量、投与速度、投与可否、抗菌薬選択、VTE予防薬、抗凝固再開時期、施設採用品、看護運用、CDS発火条件は追加していません。詳細は [audit/JAPANESE_KNOWLEDGE_13_PATCH_APPLICATION_REPORT_20260605.md](audit/JAPANESE_KNOWLEDGE_13_PATCH_APPLICATION_REPORT_20260605.md)。Preview 実出力の承認は未完了であり、医療従事者向け完成文書ではありません。
