---
drug_key: ozagrel
japanese_name: オザグレルナトリウム
english_name: ozagrel sodium
generic_name: ozagrel sodium
synonyms: "オザグレルNa|カタクロット|キサンボン|ozagrel"
drug_class: TXA2合成酵素阻害薬・抗血小板/脳血管攣縮関連
domain: acute_stroke_sah
priority: S
...
## 1. この薬剤を追加する理由
脳血栓症急性期とSAH術後脳血管攣縮の2文脈で重要。出血リスク、脳塞栓との鑑別、rt-PA等との併用混同が危険。
...
1. 「この薬剤はacute_stroke_sah文脈で重要だが、製品単位のPMDA確認と院内採用品確認が未完了」と明示する。
...
- PMDA_OZAGREL_SAFETY_2019: PMDA資料 オザグレルナトリウム関連 安全対策資料  \
  URL: https://www.pmda.go.jp/files/000230548.pdf  \
  使用箇所: オザグレルのSAH術後脳血管攣縮、脳血栓症急性期、出血リスク文脈の確認  \
  限界: 個別販売名の最新添付文書そのものではない。製品単位ではPMDA医療用医薬品検索で再確認必須。

## 7. PMDA検索クエリ候補

```text
オザグレルナトリウム OR ozagrel sodium OR ozagrel sodium OR オザグレルNa|カタクロット|キサンボン|ozagrel
```
